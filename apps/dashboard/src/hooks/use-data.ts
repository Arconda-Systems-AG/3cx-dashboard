...
export interface LiveProblemQueue {
  ...
  acute: boolean;
  atLimit: boolean;
  isSlaProblem: boolean;
  acuteProblems: string[];
  atLimitText: string | null;
  slaProblemText: string | null;
}
...
export function useGroups() {
  return useSWR<{ groups: Array<{ id: number; name: string; memberNumbers: string[] }> }>("/api/groups", fetcher, { refreshInterval: 300_000 });
}
